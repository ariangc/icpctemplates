// Note: If using type 2 (up to k, number of blocks) with 
// k~n, then it might be the case that res(i) is convex (for 
// i in [1,k]). In this case binary search can be done, pick 
// a slope X, rotate so that the slope is 0, and find i such 
// that res(i) is best (using i blocks is best). If i > k or 
// i < k adjust the slope accordingly.
