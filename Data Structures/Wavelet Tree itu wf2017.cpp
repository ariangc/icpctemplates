// nilehmann's Wavelet Tree
// Structure that splits the array recursively not by
// position, but by element values
...
struct WaveTree {
  ...
  // sigma = size of the alphabet
	WaveTree(vector<int> &A, int sigma) : 
		C(sigma*2), s(sigma) {
  ...
  void build(iter b, iter e, int L, int U, int u) {
    ...
    // C[u][i] counts amount in left side in [0,i)
  ...
  // Count amount of values in positions [i,j) 
  // that fall in range [a,b]
	mutable int L, U;
	int range(int i, int j, int a, int b) const {
		if (b < a or j <= i) return 0;
		L = a; U = b;
		return range(i, j, 0, s-1, 1);
	}
